from sqlalchemy import Column, Integer, String, create_engine, ForeignKey, Table, NUMERIC
...
# ---------------------------
params={'text': 'Сварщик', 'employment': 'full', 'education_level': 'secondary', 'business_trip_readiness': 'ready', 'area': '43'}
# ---------------------------
...
for i in session.query(Skill_req, Skills).filter(Skill_req.id==Skills.id, Skill_req.request==last_ses_id).all():
